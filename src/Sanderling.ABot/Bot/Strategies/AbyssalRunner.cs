...
namespace Sanderling.ABot.Bot.Strategies
{
	internal class AbyssalRunner : IStrategy
	{
		...
		private readonly (string, int)[] requiredCargoContent = new[]
		{
			("Mobile Tractor Unit", 0),
			("Nanite Repair Paste", 0),
			("Raging Exotic Filament", 3),
			("Scourge Fury Light Missile", 0),
			("Scourge Precision Light Missile", 0),
		};
				...
				if (currentState.MoveToNext)
				{
					switch (currentState)
					{
						case ShipCheckingState _:
							...
							break;
						case ReloadAtStationState reloadState:
