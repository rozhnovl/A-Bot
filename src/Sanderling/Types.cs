...
class VolatileProcess
{
	...
	static Stopwatch generalStopwatch = System.Diagnostics.Stopwatch.StartNew();

	Queue<ReadingFromGameClient> readingFromGameHistory = new Queue<ReadingFromGameClient>();


	string ToStringBase16(byte[] array) => BitConverter.ToString(array).Replace("-", "");


	Dictionary<int, SearchUIRootAddressTask> searchUIRootAddressTasks = new Dictionary<int, SearchUIRootAddressTask>();
